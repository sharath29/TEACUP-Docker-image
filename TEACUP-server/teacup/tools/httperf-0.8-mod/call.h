...
typedef struct Call
  {
	...
	/* this is used by call stats */
	int file_size;
	int record_conn_time;
	char id_hdr[28]; 
	...
	char * content;         /* Used for reply-verification only */
      }
    reply;
  }
Call;
 ...
 * this is used by call_stats*/
extern int inc_call_next_id(int n);

/*get the current next_id value */
extern int get_call_next_id();
...
#define call_set_version(c, ver) (c)->req.version = (ver);
