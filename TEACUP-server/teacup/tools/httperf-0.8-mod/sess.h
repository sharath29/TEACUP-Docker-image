...
typedef struct Sess
  {
    ...
    SSL *ssl;		/* SSL session (or NULL) */
		int use_ssl; /* use ssl=1,don't use ssl=2,default=other */
#endif /* HAVE_SSL */
    u_int failed : 1;	/* did session fail? */
    int num_requests;   /* The number of requests in this session*/
		int port; /* port to use for connections, 0 => use default */
		const char *server; /*server to use for connections, NULL => use default */
  }
Sess;
