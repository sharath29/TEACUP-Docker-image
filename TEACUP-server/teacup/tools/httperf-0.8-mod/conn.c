...
conn_init (Conn *conn)
{
  ...
  if (param.lb_ports.num_ports > 0)
    {
      conn->port = param.lb_ports.port[param.lb_ports.cur_port];
      param.lb_ports.cur_port = 
	(param.lb_ports.cur_port + 1)%param.lb_ports.num_ports;
    }
  else
